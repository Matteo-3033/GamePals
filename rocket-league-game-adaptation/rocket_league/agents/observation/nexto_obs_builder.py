...
def rotation_to_quaternion(m: npt.NDArray[np.float32]) -> npt.NDArray[np.float32]:
    trace = np.trace(m)
    q = np.zeros(4)
...
def encode_gamestate(state: RLGameState) -> list[float]:
    state_vals: list[float] = [0, state.blue_score, state.orange_score]
...
class NextoObsBuilder(ObsBuilder):
    ...
    def __init__(self, n_players: int = 2):
        ...
        self._boost_locations = np.array(common_values.BOOST_LOCATIONS)
    ...
    def build_obs(
        ...
        current_obs = self.batched_build_obs(
            np.expand_dims(encode_gamestate(game_state), axis=0)
        )

        for i, p in enumerate(game_state.players):
            if p == player_state:
                self.add_actions(current_obs, previous_action, i)
                return current_obs[i]

    @staticmethod
    def _quats_to_rot_mtx(quats: npt.NDArray[np.float32]) -> npt.NDArray[np.float32]:
    ...
    def convert_to_relative(
        q: npt.NDArray[np.float32], kv: npt.NDArray[np.float32]
    ) -> None:
    ...
    def batched_build_obs(
        self, encoded_states: npt.NDArray[np.float32]
    ) -> list[
    ...
    def add_actions(
        self, obs: Any, previous_actions: npt.NDArray[np.float32], player_index: int
    ) -> None:
        q, _, _ = obs[player_index]
